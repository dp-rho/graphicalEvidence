...
gen_params_G_Wishart <- function() {
  g_mat <- read.csv(file.path(
    package_dir, 'test_params', 'G_mat_q_50_n_100_delta_15_banded_param_1.csv'
  ), header=FALSE)
  scale_mat <- read.csv(file.path(
    package_dir, 'test_params', 'Scale_mat_q_50_n_100_delta_15_banded_param_1.csv'
  ), header=FALSE)
  S_mat <- read.csv(file.path(
    package_dir, 'test_params', 'S_mat_q_50_n_100_delta_15_banded_param_1.csv'
  ), header=FALSE)
  X_mat <- read.csv(file.path(
    package_dir, 'test_params', 'X_mat_q_50_n_100_delta_15_banded_param_1.csv'
  ), header=FALSE)
  start_gibbs <- read.csv(file.path(
    package_dir, 'test_params', 'start_gibbs_q_50_n_100_delta_15_banded_param_1.csv'
  ), header=FALSE)
