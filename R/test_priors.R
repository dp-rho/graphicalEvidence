...
gen_params_Wishart <- function() {
  X_mat <- read.csv(file.path(
    package_dir, 'test_params', 'X_mat_Wishart_q_50_n_75_alpha_100.csv'
  ), header=FALSE)
  scale_mat <- read.csv(file.path(
    package_dir, 'test_params', 'Scale_mat_Wishart_q_50_n_75_alpha_100.csv'
  ), header=FALSE)
...
test_Wishart <- function(num_runs) {
  ...
  estimated_marginal_store <- evidence(
    ...
    runs=num_runs, alpha=100, V=params$scale_mat,
  )
