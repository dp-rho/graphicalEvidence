...
gen_params_BGL <- function() {
  X_mat <- read.csv(file.path(
    package_dir, 'test_params', 'X_mat_BGL_q_25_n_50_lambda_5.csv'
  ), header=FALSE)
...
gen_params_Wishart <- function() {
  ...
  scale_mat <- read.csv(file.path(
    package_dir, 'test_params', 'Scale_mat_q_50_n_125_alpha_140.csv'
  ), header=FALSE)
