...
gen_params_G_Wishart <- function() {
  g_mat <- read.csv(file.path(
    package_dir, 'test_params', 'G_mat_q_100_n_125_delta_10.csv'
  ), header=FALSE)
  scale_mat <- read.csv(file.path(
    package_dir, 'test_params', 'Scale_mat_q_100_n_125_delta_10.csv'
  ), header=FALSE)
  X_mat <- read.csv(file.path(
    package_dir, 'test_params', 'X_mat_q_100_n_125_delta_10.csv'
  ), header=FALSE)
  return(list(
...
test_G_Wishart <- function(num_runs) {
  ...
  estimated_marginal_store <- evidence(
    ...
    runs=num_runs, alpha=10, V=params$scale_mat, G=params$g_mat,
    print_progress=TRUE
  )
