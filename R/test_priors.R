...
gen_params_GHS <- function() {
  X_mat <- read.csv(file.path(
    package_dir, 'test_params', 'X_mat_GHS_q_50_n_150_lambda_140.csv'
  ), header=FALSE)
  return(list(X_mat=X_mat))
}
...
test_G_Wishart <- function(num_runs) {
  ...
  estimated_marginal_store <- evidence(
    ...
    run=num_runs, alpha=2, V=params$scale_mat, G=params$g_mat
  )
...
test_BGL <- function(num_runs) {
  ...
  estimated_marginal_store <- evidence(
    ...
    run=num_runs, lambda=140, print_progress=TRUE
  )
